...
if [ -z "$5" ]; then
    echo "Usage; sbatch $0 [STAR index] [white list] [R1] [R2] [scTE nointron index]" >&2
    exit 1
fi
...
CMD="scTE -i ${BAM} -o ${BASE}_scTE_nointron -p 10 -x ${IDX} -CB CB -UMI UB 2>&1 >> ${LOG}"

${CMD}

if [ $? -ne 0 ]; then
    echo "Error with scTE" >&2
    exit 1
else
    echo "Done"
fi
