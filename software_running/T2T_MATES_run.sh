...
if [ -z "$6" ]; then
    echo "Usage; sbatch $0 [STAR index] [white list] [R1] [R2] [TE_nooverlap.bed] [TE_nooverlap.csv]" >&2
    exit 1
fi
...
OUTDIR="${CURRDIR}/${BASE}_MATESprep"
...
LIB=$(basename ${BAM} _STAR_10x\.bam)
BASE="${LIB}_MATES_exclusive"
LOG="${BASE}.log"

cd ${BASE}

echo "${LIB}" > samplelist.txt
echo "../${BAM}" > bam_path.txt
echo "../${OUTDIR}/Solot.out/GeneFull/filtered/barcodes.tsv" > cb_path.txt
if [[ "${BED}" == /* ]];then
    ln -sf ${BED}
    ln -sf ${CSV}
else
...
echo "#!/bin/env python" > ${SCRIPT}
echo "from MATES import MATES_pipeline" >> ${SCRIPT}
echo "mates = MATES_pipeline('exclusive','10X','samplelist.txt','bam_path.txt',threads_num=10,bc_path_file='cb_path.txt',ref_path='TE_nooverlap.csv')" >> ${SCRIPT}

python ${SCRIPT}

if [ $? -ne 0 ]; then
    echo "Error with MATES" >&2
    exit 1
else
    cd ..
    echo "Done"
